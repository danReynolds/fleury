...
import PreferencesTestCode from '../../../components/PreferencesTestCode.astro';
...
Work widget by its type and key, then change only its controls. Use **Run test**
to watch the actions and assertions, or type into Work and use Tab to explore:
...
<PreferencesTestCode slot="code-1" />
<FleuryExample slot="demo" id="testing.preferences" test="preferences" cols={32} rows={11} interactive />
</CodeDemoSplit>
