...
This — a text field over a filtered list with a live count. It's the finished
app, running in your browser; **type into it** and watch the list narrow:

<FleuryExample id="tutorial.filter" cols={40} rows={16} interactive />

## 1. A static list

Start with a stateless screen that just renders some data — replace what the
generator put there:
