...
| G4 | ~~**No true input→paint latency** on either surface (`commandToFrameUs` is an in-VM proxy; the wire never correlates injected input → resulting frame)~~ **BUILT (serve surface)** | Input responsiveness is the headline UX axis for WAN-SSH and served browser | **`input-latency` scenario on the live-wire harness** (`fleury benchmark serve-wire-live`): the scenario app idles until a key arrives and repaints deterministically per keystroke; the headless client injects keys **closed-loop** (arm → timestamp → INPUT_EVENT 0x14 → await the PLAN 0x12 it provokes), 50 samples after warmup. **Structural gate**: every key must be answered by exactly one PLAN within 2s — a miss or an unsolicited plan exits non-zero (a broken input path cannot pass). The latency p50/p95 axes are warn-only baseline entries (live-socket wall-clock; cadence precedent) — loopback today ≈ **p50 1.4 ms / p95 ~2 ms**. Terminal-surface (PTY) input latency remains open. |
...
| G6 | ~~**No dart2js bundle-size gate**~~ **BUILT** | First-load weight regresses trivially (a stray import = 100s of KB) | **`bundle_size_gate` (`fleury benchmark bundle-size [--gate]`)** measures the SHIPPED served-browser client (`remote_client.dart.js`, served at `GET /client`) — raw + gzip of the embedded bytes, no recompile. Today **314 KiB raw / 77 KiB gzip** (a reasonable dart2js bundle, not bloated — confirmatory, not a fix). Threshold gate (512 / 160 KiB) is SDK-tolerant — catches a stray-import bloat, ignores normal dart2js drift. |
...
4. ~~G1 live-serve-wire harness~~ **DONE** — `serve_wire_live_*` (see G1). Surfaced that semantics dominate the **raw** serve wire 5–13×, which on investigation (§3, Tier 1) is **not** a delivered-cost problem — the diff encoder + DEFLATE already keep it flat at 10–38 B/frame, now guarded by `serve-semantics-gate`. The next real serve/perf levers are the coverage tools (G2/G6), not semantics. ~~G4~~ (**DONE** — the `input-latency` scenario, see G4) and G5 layer onto this harness.
...
7. ~~G6 bundle-size gate~~ **DONE** (`bundle-size --gate`, 314/77 KiB, confirmatory). **G3 allocation gate** remains — regression insurance.
