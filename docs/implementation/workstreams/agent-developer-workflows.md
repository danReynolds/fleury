...
- The current-cycle example demo-app workflow scenario is defined in
  [demo-app-scenario.md](../demo-app-scenario.md).
- The first runnable demo-app package skeleton now lives at
  [packages/fleury_example_console](../../../packages/fleury_example_console).
- M1.4 demo app v0 is complete for the current cycle: it exercises app
  ...
  targeted tests. The demo app's own sidebar and diagnostic semantic actions
...
- `fleury_widgets` now includes a first protocol-neutral approval surface:
  ...
  submit/cancel actions and demo-app command coverage.
- `fleury_widgets` now includes a protocol-neutral `MessageList` transcript
  ...
  not following the tail, accessibility fallback state, and demo-app
  Transcript screen adoption. The demo app now assigns durable transcript
...
- `fleury_widgets` now includes a protocol-neutral `ToolCallCard` execution
  ...
  copy/export, semantic cancel, and demo-app Process screen adoption.
- `fleury_widgets` now includes a protocol-neutral `TaskGraph` plan/task
  ...
  copy/export, and demo-app Overview adoption.
- `fleury_widgets` now includes protocol-neutral `ModelStatusBar` and
  ...
  accessibility fallback state, and demo-app Overview adoption.
- `fleury_widgets` now includes a protocol-neutral `FileMentionPicker` for
  ...
  sanitized mention export, accessibility fallback state, and demo-app
  Transcript composer adoption.
- The demo app Transcript composer now uses `CompletionTextInput` for
...
- `fleury_widgets` now includes a protocol-neutral `ConversationNavigator` for
  ...
  accessibility fallback state, and demo-app Transcript adoption.
...
- `fleury_widgets` now includes a protocol-neutral `ContextPanel` for
  ...
  refresh-stable selected context, and demo-app Overview adoption.
- `fleury_widgets` now includes a protocol-neutral `TraceTimeline` for
  ...
  state, and demo-app Diagnostics adoption.
- `fleury_widgets` now includes a protocol-neutral `PatchReview` for code-review
  ...
  fallback state, refresh-stable selected file, and demo-app Changes adoption
...
- `fleury_widgets` now includes `WorkflowSnapshot`, `WorkflowSummary`, and
  ...
  conversations, trace events, patch files, and logs. The demo app Overview
...
- Example demo-app workflows first, then later Dune/`dune_cli` workflows that
...
- [x] ADW.1 Define reference agent workflow model.
  ...
  - Evidence:
    ...
    [demo app Overview workflow summary](../../../packages/fleury_example_console/lib/fleury_example_console.dart),
  ...
  - Notes: Keep the model framework-shaped, not tied to one provider. Current
    ...
    demo-app Overview adoption. The follow-up fallback slice adds an
...
- [x] ADW.2 Build demo-app workflow scenario spec.
  ...
  - Notes: This is intentionally protocol-neutral. Tool calls, approvals, ACP,
    ...
    validation reran demo-app analyze and workflow tests successfully.
    Current Phase 2 demo-app slice adds an Indexed Logs screen that builds and
    ...
    current semantic-action settling slice makes the demo app's app-authored
...
- [x] ADW.3 Add agent workflow semantics.
  ...
  - Evidence:
    ...
    [demo app Transcript tests](../../../packages/fleury_example_console/test/demo_console_test.dart).
  - Notes: Structured actions should avoid screen scraping. Current slice makes
    ...
    demo app now keeps transcript event IDs stable and holds the message-list
...
- [x] ADW.4 Build first widget set.
  ...
  - Notes: First slice is `ApprovalPrompt`, intentionally protocol-neutral and
    ...
    tail-follow preservation for live transcripts. Current demo-app hardening
    ...
    into the demo-app composer. Current conversation-navigation slice adds
    ...
    preservation, sanitized copy, and semantic activation in the demo-app
    Transcript screen.
    ...
    activation in the demo-app Diagnostics screen. Current effect-history
    ...
    the demo-app Changes screen. It composes `DiffView` instead of replacing
...
- The first demo app is a developer operations console, not an ACP console.
- The demo app is complete as a pressure harness, not as polished public
...
- Permission and approval flows must integrate with security policy and
  replay. The first approval prompt now has semantic actions and demo-app
