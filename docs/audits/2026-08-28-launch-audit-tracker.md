...
- [x] **15.a** `P1` ✎ Supervisor's 300 ms signal forward collapses the documented 5 s grace — `dev_bootstrap.dart:313`
  ...
  **Notes:** PARTIAL (1fe211ce): the worse mechanism is fixed — restore() now holds a no-op SIGINT/SIGTERM shield until cooked mode + final flush are done, so a forwarded signal can no longer kill the process raw mid-restore (test via a signal-watcher seam, red without the shield). The 300 ms provenance guess itself (positive ack over the VM service) is a design call — open. LANDED (P3 merge): the child posts fleury.signalAck over the VM service; the supervisor cancels the forward on ack; the timer is a 2 s wedged-child backstop. PTY-pinned: a 3 s teardown under tty Ctrl+C exits with its own code (was 130 / death by forwarded signal). Residual race documented (ack microseconds before a respawn).
...
- [ ] **D2** The harness diverges from production in three ways, each hiding a specific P1 — `fleury_tester.dart:107, 960, 985`
  ...
  **Notes:** PARTIAL — the latch divergence is closed (see **11.e**); the `textPolicy` knob (4.a) and the entry-boundary opt-out (7.a) are untouched. The general lesson the fix encodes: the harness now shares the host wiring function rather than reimplementing it, so this class of divergence cannot be introduced by editing only one side. CLOSED (branch `audit/p7-gates-prod`): `FleuryTester.textPolicy` + `testWidgets(textPolicy:)` in both packages, and `testWidgetsOnBothTextPolicies` registers one body on BOTH surfaces (ambiguous_wide_surface_test converted: 8 cases → 16, the spec halves are new). Entry repaint boundaries now default to production (`overlayRepaintBoundaries`, opt-out for boundary-counting tests). Whole-suite effect: exactly 3 tests moved, all in `overlay_test.dart`'s boundary census, each opted out with a one-line reason.
...
- [ ] **D3** Two gates don't cover what their names imply — `profiling/bin/selection_gate.dart:82`; `alloc-gate` vs `framework.dart:854`
  ...
  **Notes:** CONFIRMED WORSE while fixing 12.e: alloc-gate sums only classes whose library uri starts with package:fleury, so dart:core String/List churn is structurally invisible — anyone reading it as a general allocation gate is misled. Also: wire-gate's sb6 scenario is nondeterministic (~4% spread on identical code; sb1/sb9 are byte-stable). SELECTION HALF CLOSED (branch `audit/p7-gates-prod`): the gate now drives a real `SelectionArea` — press/drag/release, Ctrl+A, Ctrl+C, Esc through a real `InputDispatcher` + `PointerRouter` on a real `TuiRuntime`. Gated counters: drag chars/cells, select-all chars/cells (2528, unchanged), painted highlight cells; invariants (a drag selects something, Esc clears to zero) hold under `--update-baseline`. Demonstrated: with `SelectionArea`'s drag edge update removed the OLD gate still reported "coverage 2528 cells matches baseline — pass". A NEW `runtime-gate` covers the other half of this item's spirit (nothing measured `runApp` → `FrameDriver` → present); it immediately found an unbounded frame spin after a paint-pass retraction — see the adjacent note below. The alloc-gate blind spot itself is still open.
...
- [x] **N12** `P2` `fleury_widgets` is 4.a's remaining half: ~50 `writeGrapheme` sites across 17 chart/table files (block elements, braille, sextants, box-drawing rules) take the spec default with no way to reach the ambient policy — none of those render objects carries a `TextPresentationPolicy`. Every one of those glyph blocks is East Asian Ambiguous, so on an ambiguous-wide terminal each chart is drawn at half its modelled resolution with the same severed-pair corruption 4.a fixed in core. Fixing it means threading `MediaQuery.textPolicyOf` into the chart render objects and making the sub-cell buffers (`HalfBlockBuffer`, `BrailleBuffer`, `OctantBuffer`, `QuadrantBuffer`, `SextantBuffer`) policy-aware — a package-sized piece, not a call-site sweep. **This is why 4.a did not make `policy` required on `writeGrapheme`/`writeText`:** ~550 call sites across four packages, and until N12 lands ~60 of them would have to pass an explicitly wrong `CellWidthPolicy.spec`.
  **Notes:** Implemented in `codex/launch-hardening-followups` (September 4; pending merge). Ambiguous-wide drawing uses an ASCII repertoire at the original cell resolution; chart labels measure and paint with the text policy. Table replay preserves measured cell spans, standalone sub-cell buffers accept a width policy, and image fallback samples a background color per cell. The 54-case rendering matrix covers both policies, Unicode labels, tooltips, data tables, images, canvas modes, policy changes and emoji lowering. See `2026-09-04-launch-hardening-fixes.md` for scope and validation.
- [x] **N14** `P1` **An anchored float's retraction spins the runtime forever.** `RenderDamageTracker.endPaintPass` counted a retraction for every registered participant whose `publishedPaintPass != paintPass` — which is true on EVERY pass after a `BoundsObserver` stops painting while staying mounted (the other `IndexedStack` tab, a route under an opaque one). `FrameDriver` answers a non-zero count by re-recording visual change and requesting another frame, so one tab switch made every frame request the next: measured **366,353 frames in the second after the switch**, unbounded, at 100% CPU. Both existing regression tests passed — they assert "at least two frames" and "the last one is a retraction", and a spin satisfies both. Found by the new `runtime-gate` on its first run.
  **Notes:** LANDED (branch `audit/p7-gates-prod`): `retractPaintFacts()` returns whether it actually withdrew a live fact (`BoundsNotifier.publish` now returns "did this change anything"), and the sweep counts only those — a re-visit of an already-retracted participant is free. 366,353 frames/s → 2 frames total, which is the documented shape (the rebuild frame, then the one that hides the float). Pinned in `bounds_retraction_frame_test.dart` (red first: 885 extra frames after the tree went quiet) and by `runtime-gate`'s exact `framesRendered: 2` plus a spin tripwire. Merged with the coalescing frame-request primitive (`RenderDamageTracker.onInvalidate`): the sweep no longer counts anything — a retraction that withdrew a live fact notifies its observers, whose invalidation lands in the paint phase and asks for the next frame (`invalidate`), and `publish(null)` on an already-withdrawn fact is a no-op, which is what stops the spin. `runtime-gate` pins the one-shot.
...
## PR review outcome (2026-09-02)

Four read-only reviewers (core, input/runtime/terminal, serve/remote/web, editing/widgets) read the diff against main. Confirmed findings and what was done:

- **Fixed:** 6.c retraction never reached the screen in the real runtime (frame driver now re-arms a frame after a retraction; runtime-level test); TerminalSessionScope unreachable from overlay entries (moved above the Overlay); sanitizer lone-high-surrogate leak; unbounded restore() shield (bounded: second signal or 2 s); serve dying on greeting a closed app socket (addStream + observed `done`); §14.4 repeat bypass on split key/text surfaces (`TextInputEvent.repeat` end to end); paste canonicalized once; CRLF at the model boundary; ConstrainedBox intrinsics; CodeView/JsonView cap under bounded parents; Menu/Select label widths in cells; RenderBorder paint follows layout's frame decision; cell-effect box guard; supervisor ack attribution across a respawn; dispatcher per-key allocation; browser banner reconnect for deliberate closes; first-probe doc bound.
- **Not changed, by decision:** `_RenderScrollView` / `AspectRatio` / `RenderClip` now report their child's intrinsic size (was 0; both answers are approximations — Flutter throws); the debug console's per-cluster projection in `_fit` (debug surface); `BoundsObserver._publish` walking to the root per paint (bounded by observer count); `newlyVisible` not cleared before the lazy tail re-walk (safe: the tail window is a superset — commented); the provisional INIT makes serve↔app lockstep-only — an older app would take the greeting as its handshake (pre-launch there are no older apps; bears on **16.g**).
- **Adjacent, unchanged:** `TextEditingModel.snapOffsetToGraphemeBoundary` is O(offset) — a latent quadratic for mid-document edits in large texts; `_attachPump` must stay synchronous (unenforced); close-reason strings are unguarded literals (123-byte cap); the document-level pointerdown also fires for the semantics element (wants a real-browser check); latch handover is global to any live ticker (by RFC).
