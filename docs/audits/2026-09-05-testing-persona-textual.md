...
**Observed.** The enum mixes general UI concepts and rich domain concepts. The DOM presenter already projects several rich roles into ordinary ARIA roles: `patchFile` becomes `listitem`; `toolCall` and `tokenMeter` become `status`. Semantic coverage also treats domain containers differently from readable content. This is an existing inspection/accessibility boundary, not simply accidental unused names. [Enum, lines 40–105](../../packages/fleury/lib/src/semantics/semantics.dart#L40), [DOM role projection, lines 509–571](../../packages/fleury_web/lib/src/semantics/semantic_dom_presenter.dart#L509), [coverage, lines 263–288](../../packages/fleury/lib/src/semantics/semantic_coverage.dart#L263)

The suite audit found a more immediate inconsistency: MultiSelect options claim checkbox meaning and expose checked state, yet lack the boolean setting capability offered by other checkbox-like controls. The RFC addresses this in shared widget semantics rather than a tester workaround. [Suite audit, lines 89–105](2026-09-05-testing-controls-suite-audit.md#L89), [RFC, lines 313–331](../implementation/rfc-testing-controls.md#L313)
...
**Observed.** The table test first establishes that row 5000 is absent from the published tree, then calls table `setValue(5000)` to select and reveal it. Out-of-range indices clamp. Separate keyboard tests verify that PageDown moves the viewport and selection. The proposed facade preserves that same scalar operation. [Table tests, lines 166–223](../../packages/fleury_widgets/test/data_table_test.dart#L166), [RFC, lines 238–248](../implementation/rfc-testing-controls.md#L238)
...
**Observed.** The same FleuryTarget proposes 12 operations, with runtime capability checks. Even `fill` requires a particular role family plus focus and value capabilities, and desired-state checking has a stronger postcondition than plain `setValue`. The RFC already promises capability-specific diagnostics. [RFC, lines 166–197](../implementation/rfc-testing-controls.md#L166), [operation contracts, lines 294–331](../implementation/rfc-testing-controls.md#L294)
