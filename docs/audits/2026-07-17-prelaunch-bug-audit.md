...
| --- | --- | --- |
| A — release mechanics | pub publish failure; getting-started snippet fixture | blocks/protects launch itself — **DONE 2026-07-17** (PR #127; the package:test import was already fixed by the fleury_test extraction; sole remaining publish blocker = stdio git dep, a release decision) |
| B — framework core | GlobalKey cluster (4 findings, one PR) | double-repro'd core crashes — **IMPLEMENTED 2026-07-17** (PR #128; re-verified live post-#103 first — #103 fixed a third, different steal case) |
| C — focus & input | focus.dart pair, dispatcher sequences, samples 'q' | Ctrl+C bypass + broken demos — **IMPLEMENTED 2026-07-17, PR #123** (+5th bug found: DataTable/Tree type-ahead swallowed printables → additive `typeahead` flag; navigator pop focus-restore now post-frame by contract) |
| D — process lifecycle | effects/ 3×P1 + task.dart:394 | agent-story credibility — **IMPLEMENTED 2026-07-17, PR #124** (detached start modes now rejected by contract) |
...
| F — data entry | FormPanel decimal, DatePicker, FilePicker, NaN charts | visible data corruption — **IMPLEMENTED 2026-07-17, PR #126** (FilePicker `initialDirectory` contract: error row instead of throw) |
