...
// function.
//
// This diagnostic has no regression threshold. Invalid arguments or VM-service
// failures still return a nonzero exit code.
//
...
Future<void> main(List<String> args) async {
  ...
  for (final arg in args) {
    if (parsePositiveIntFlag(arg, 'frames') case final v?) {
      frames = v;
    } else if (parseIntFlag(arg, 'warmup') case final v?) {
      warmup = v;
    } else if (parsePositiveIntFlag(arg, 'stacks') case final v?) {
      stacks = v;
    } else if (parsePositiveIntFlag(arg, 'depth') case final v?) {
      depth = v;
    } else if (parsePositiveIntFlag(arg, 'auto') case final v?) {
      auto = v;
  ...
  if (warmup < 0) {
    stderr.writeln('invalid --warmup: expected a nonnegative integer');
    exitCode = 64;
    return;
  }
  ...
  try {
    ...
    for (final target in targets) {
      ...
      late int end;
      try {
        start = (await service.getVMTimelineMicros()).timestamp!;
        for (var i = 0; i < frames; i++) {
          frame();
        }
        end = (await service.getVMTimelineMicros()).timestamp!;
      } finally {
